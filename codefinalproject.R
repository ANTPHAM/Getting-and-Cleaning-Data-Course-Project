...
# change the names of other columns
varnames<-featuremeasure[featurenum,]
varnames<-tolower(gsub("[^[:alpha:]]","",varnames$V2))
setnames(Mergedat, old=c(3:68), new=c(varnames))
dim(Mergedat)


#5.From the data set in step 4, creates a second,independent tidy data set with the average of each variable for each activity and each subject.
tidydat<- Mergedat %>% group_by(activitylabel,subject) %>% summarise_each(funs(mean))
write.table(format(tidydat,scientifique=T),"mean_tidy_dat.txt",row.names=T, col.names=T, quote=2)
